...
1. 考虑前序遍历与后序遍历的关系。前序遍历为根→左→右，后序遍历为左→右→根。
   若将前序遍历改为根→右→左，则这个变体前序遍历的结果在反转后就是后序遍历。
2. 令 curr 指向当前节点，temp 指向 curr 右子树的最左节点
3. 若 curr 没有右子树，则根据变体前序遍历的规则，直接将 curr 放入结果数组，并令 curr=curr->left。处理 curr 的左子树。
4. 若 curr 有右子树，则根据莫里斯遍历的思想，令 temp 指向 curr 右子树的最左节点，并使 temp->left=curr。
   ...
   此时，可以通过 temp->left=curr 这条回边，回到 curr，然后令 curr=curr->left，继续处理 curr 的左子树。
